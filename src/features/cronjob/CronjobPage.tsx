...
export function CronjobPage() {
  ...
  return <section className="cronjob-page">
    <div className="workspace-header"><div><div className="breadcrumb">服务器 / <span>计划任务</span></div><h1>计划任务</h1><p>crontab 写入、systemd timer 查看与一次性执行</p></div><div className="workspace-header__actions"><Button variant="ghost" onClick={() => setNotificationOpen((current) => !current)}><Bell size={14} /> 报告通知</Button><Button variant="ghost" onClick={() => void exportJobs()} disabled={cronjobs.isFetching || !cronjobs.data}><Download size={14} /> 导出任务</Button><Button variant="ghost" onClick={() => importInputRef.current?.click()} disabled={cronjobs.isFetching}><Upload size={14} /> 导入任务</Button><input ref={importInputRef} type="file" accept="application/json,.json" hidden onChange={(event) => void importJobs(event)} /><Button variant="secondary" onClick={() => cronjobs.refetch()} disabled={cronjobs.isFetching}><RefreshCw className={cronjobs.isFetching ? "spin" : ""} size={14} /> 刷新任务</Button></div></div>
    <CronTabs serverId={serverId} />
