...
> 后续开发或交接请先读 [HANDOFF.md](HANDOFF.md)：它包含完整文档阅读顺序、当前进度、测试服务器凭据、当前第一任务和大阶段视觉验收流程。
...
- 真实 lsblk/findmnt/df/fstab 磁盘与挂载快照，按磁盘/分区/RAID/LVM/其他设备汇总拓扑摘要，受控挂载/卸载和带备份回滚的 fstab 维护
...
- OpenAI-compatible AI 供应商密钥链配置、真实 `/models` 探测、可取消 SSE 流式响应、只读服务器概览/网站/Docker/安全/进程快照智能体、按供应商隔离的本地会话恢复/清理与可选 MCP stdio 工具
